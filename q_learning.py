...
class Model:
    ...
    def sample_action(self, s, eps):
        ...
        if p < eps:
            ...
            action_index = np.argmax(self.predict(s, self.env.action_space.get_legal_action_indices(
...
def play_one(env, model, eps):
    ...
    while not done and n_iterations < 2000:
        # if we reach 2000, just quit, don't want this going forever
        # the 200 limit seems a bit early
        action_index = model.sample_action(state, eps)
        assert action_index in env.action_space.get_legal_action_indices(state)
        ...
        if done:
            ...
            q_value = env.get_q_value(action_index, cached_prev_state, reward, max(future_rewards))
        model.update(cached_prev_state, action_index, q_value)
...
def test_model(env, model, plot=False):
    ...
    while env.day_number < (env.day_chunk - 1):
        ...
        net_load_list[env.day_number - 1].append(prev_observation[1] - prev_observation[0])
    max_savings = -1000
    best_day = -1
...
def main():
    ...
    for n in range(N):
        ...
        total_reward, number_of_hours_lasted = play_one(env, model, eps)
        total_rewards[n], number_of_hours_lasted_lst[n] = total_reward, number_of_hours_lasted
        if n > 0 and n % 50 == 0:
            current_saving, hours_lasted = test_model(env, model, True)
